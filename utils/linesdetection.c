...
void
draw_line(int *pixels, long int w, long int h, long int x1, long int y1, long int x2, long int y2, Uint32 color)
{
    int i, dx, dy, maxmove;
    int d, dinc1, dinc2;
    int x, xinc1, xinc2;
        ...
        for (size_t i = 0; i < (size_t)w; i++) {
            ...
            if (r == 0 && g == 0 && b == 0) {
                for (size_t theta = 0; theta < ANGLE; theta++) {
                    k = (long unsigned int)((double)j * cos((double)theta * PI/180) + (double)i * sin((double)theta * PI/180));
                    if (j == 420 && i == 278) {
                        printf("%lu, %lu, %lu, %lu\n", i, j, k, theta);
                        // printf("%i\n", r);
                    }
        ...
        for (size_t j = 0; j < ANGLE; j++) {
            if (mat[i * ANGLE +j] > 400) {
                ...
                if (x2 < 10 && y2 < 10) {
                    ...
                    printf("k: %lu, m: %f, n: %f, rho: %lu, theta: %lu, x0: %li, y0: %li, x1: %li, y1: %li, x2: %li, y2: %li\n", mat[i * ANGLE +j], m, n, i, j, x0, y0, x1, y1, x2, y2);
                }
                if (i == 424 && j == 89) {// kl < 50 ){
                    printf("k: %lu, m: %f, n: %f, theta: %lu, x0: %li, y0: %li, x1: %li, y1: %li, x2: %li, y2: %li\n", mat[i * ANGLE +j], m, n, j, x0, y0, x1, y1, x2, y2);
                    kl +=1;
                }
