...
Points_Array *
get_intersection_points(Points_Array *array, long int w, long int h)
{
    ...
    for (size_t i = 0; i < array->len; i += 2) {
        // for (size_t j = 0; j < array->count_v; j += 2) {
        if (parametricIntersect(array->array[i], array->array[i + 1],
                                array->array[i + 2], array->array[i + 3], &x,
                                &y)) {
            if (x >= 0 && x < w && y >= 0 && y < h) {
                printf("(%lu, %lu)\n", x, y);
...
get_corners(SDL_Surface *image)
{
    ...
    for (size_t i = 10; i < image->h - 11; i++) {
        for (size_t j = 10; j < image->w - 11; j++) {
            ...
            if (m > 1500 && d > 1000) {
                // printf(
                //   "|_ %Lf, y: %lu, x: %lu, Gx: %Lf, Gy: %Lf, m: %Lf, o:
                //   %Lf\n", d, i, j, dx[i * out->w + j], dy[i * out->w + j], m,
                //   o);
            }
...
main(int argc, char **argv)
{
    ...
    // image_temp = detect_lines_and_rotate(pixels, image_temp->w,
    // image_temp->h,
    //                                      image_temp->format);
    ...
    for (size_t i = 0; i < intersect_arr->len; i++) {
