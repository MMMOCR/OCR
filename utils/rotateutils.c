...
rotate_image(SDL_Surface *image, double angle)
{
    ...
    if (fmod(angle, (double)PI / 2) == 0) {
        ...
        max_edge =
            (int)sqrt(image_w * image_w + image_h * image_h) + 1;
    }
    ...
    if (fmod(angle, (double)PI / 2) == 0) {
        for (size_t i = 0; i < max_edge; i++) {
            for (size_t j = 0; j < max_edge; j++) {
                x = round((j - half_image_w) * cos(angle) - (i - half_image_h) * sin(angle)) + half_image_w; 
                y = round((j - half_image_w) * sin(angle) + (i - half_image_h) * cos(angle)) + half_image_h;
                if (x < 0 || x > image_w) {
                    continue;
                }
                if (y < 0 || y > image_h) {
        ...
        for (size_t i = 0; i < max_edge + (max_edge - image_h)/2; i++) {
            for (size_t j = 0; j < max_edge + (max_edge - image_w)/2; j++) {
                ...
                SDL_GetRGB(pixel, image->format, &r, &g, &b);
                pixel = SDL_MapRGB(rotated_surface->format, r, g, b);
                put_pixel(rotated_surface, j + (max_edge - image_w)/2, i + (max_edge - image_h)/2, pixel);
            }
        }
