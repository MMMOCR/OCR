...
treshold(long int len, Uint32* pixels, int override_treshold)
{
    ...
    for (size_t i = 0; i <= 255; i++) histogram[i] = 0;
    ...
    else
    {
        ...
        for (size_t i = 0; i <= 255; i++)
        {
            ...
            for (size_t j = 0; j <= 255; j++)
            {
                ...
                {
                    if (histogram[j] != 0) 
                        sumvarbg += histogram[j]*
                            (float)(j-meanbg)*(float)(j-meanbg);
                }
                else
                {
                    if (histogram[j] != 0)
                        sumvarfg += histogram[j]*
                            (float)(j-meanfg)*(float)(j-meanfg);
                }
            }
...
void
event_loop(SDL_Renderer* renderer, SDL_Texture* colored,
...
load_image(const char* path)
{
    ...
    SDL_Surface* format = SDL_ConvertSurfaceFormat(image,
...
main(int argc, char** argv)
{
    ...
    SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, colored_surface);


    surface_to_grayscale(colored_surface);

    int threshold = treshold(colored_surface->w * colored_surface->h, colored_surface->pixels, 0);

    back_to_black(colored_surface, threshold);

    SDL_Texture* grayscale_texture = SDL_CreateTextureFromSurface(renderer, colored_surface);
