...
multiple(long int w, long int h, SDL_Surface* surface)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        for (size_t j = 0; j < 3; j++) {
            ...
            SDL_Surface* new = SDL_CreateRGBSurface(0, surface->w/3, surface->h/3, 32, 0, 0, 0, 0);


        SDL_BlitSurface(surface, &crop, new, NULL);

        // printf("%i %i\n", new->w, new->h);
        int threshold = otsu_treshold(new->w * new->h,
                                 new->pixels, 0);
        ret[k] = threshold;
        k++;
        }
    ...
    for (size_t i = 0; i < 3; i++) {
        for (size_t j = 0; j < 3; j++) {
            ...
            to_black(surface, (int)(ret[k]*0.5+global*0.5), y,x, hh, ww);
...
otsu_treshold(long int len, Uint32* pixels, int override_treshold)
{
    ...
    if (override_treshold != 0) {
        ...
        for (size_t i = 1; i <= 255; i++) {
            ...
            pbg = 0, pfg = 0, sumfg = 0.0, sumbg = 0.0, sumvarbg=0.0, sumvarfg = 0.0;
        }
    }
    printf("treshold: %d\n", treshold);
    return treshold;
