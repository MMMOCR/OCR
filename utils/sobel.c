...
{
    int Gx[5][5] = { {2, 2, 4, 2, 2}, {1, 1, 2, 1, 1}, {0, 0, 0, 0, 0},
        {-1, -1, -2, -1, -1}, {-2, -2, -4, -2, -2}};
    int Gy[5][5] = { {2, 1, 0, -1, -2}, {2, 1, 0, -1, -2}, {4, 2, 0, -2, -4},
        {2, 1, 0, -1, -2}, {2, 1, 0, -1, -2}};    
...
edges(SDL_Surface* surface)
{
    ...
    for (size_t i = 0; i < h; i++) {
        for (size_t j = 0; j < w; j++) {
            ...
            int test = 255*(double)(val-min)/(max-min); //set between 0 and 255
            ...
            for (int x = -1; x < 2; x++) {
                for (int y = -1; y < 2; y++) {
                    if (i+x < h && i+x > 0 && j+y < w && j+y >0)
                    {
                        ...
                        ky = y+1;
                        // printf("%d\n", ky);
                        valx = (pixels[(i+x)*h + (j+y)] >> 16 & 0xff)*Gx[kx][ky];
                        // printf("%d\n", pixels[(i+x)*h + (j+y)] >> 16 & 0xff);
                        Gx_compute += valx;
                        valy = (pixels[(i+x)*h + (j+y)] >> 16 & 0xff)*Gy[kx][ky];
                        // printf("%d\n", valy);
                        Gy_compute += valy;
                    }
                }
