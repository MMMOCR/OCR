...
enum steps {
    Binarization, Grid, Rotation, Numbers, Solving, Output
};

static char *steps[STEPS_COUNT] = { "Binarization", "Grid detection", "Rotation", "Numbers detection", "Solving", "Output" };
static char *descs[STEPS_COUNT] = { "Binarizes the image so that detecting the grid and numbers becomes easier",
                                    "Detects the grid and crops the image", "Rotates automatically the image",
                                    "Detects numbers using a neural network", "Solves the sudoku using the backtracking algorithm",
                                    "Output of the OCR sudoku solver that you can save (Tips: hold left button on the image to show the input)" };
...
static char *images[] = { "steps/binarization.png", "steps/grid_detection.png", "steps/rotation.png", "steps/numbers_detection.png", "steps/solving.png", "steps/output.png" };
static char *bins[] = { "binarization", "grid_detection", "rotation", "numbers_detection", "solving", "output" };

static char *weigths_path;
static char *image_path = "/home/rigole/Documents/rescue/sypbc/bin/images/steps/binarization.png";
...
draw_main(struct sypbc *ctx)
{
    /* Navbar */
    {
        draw_push_filled_rect(&ctx->draw, 0, 0, WINDOW_WIDTH, 83, color_rgb(27, 27, 27)); // Background
        draw_push_text(&ctx->draw, "OCR", 30, 18, ROBOTO_BOLD45, color_rgb(255, 255, 255));
        draw_push_line(&ctx->draw, 140, 28, 140, 60, 1, color_rgb(184, 184, 184));
        draw_push_text(&ctx->draw, "Sudoku solver", 160, 26, ROBOTO_REGULAR32, color_rgb(184, 184, 184));
    ...
    if (finished && is_cursor_within(ctx->input, 494, 135, 660, 430) && is_mouse_pressed(ctx->input, 1)) {
        draw_push_raw_image(&ctx->draw, image_path, 494, 135);
    } else {
        if (working) {
            draw_push_image(&ctx->draw, images[(!finished ? current_step : selected_step)], 494, 135);
        } else {
            draw_push_image(&ctx->draw, image_path, 494, 135);
            if (!task_thread && current_step == 0) {
                pthread_create(&task_thread, NULL, (void*(*)(void*))execute_task, ctx);
            }
        }
    }

    if (finished) {
        draw_push_filled_rounded_rect(&ctx->draw, 372, 630, 875, 65, 10,
                                      color_rgb(27, 27, 27));  // Description background

        vec2 dims = get_text_dim(descs[selected_step], ROBOTO_REGULAR16);
        draw_push_text(&ctx->draw, descs[selected_step], 810 - dims.x / 2, 655, ROBOTO_REGULAR16, color_rgb(184, 184, 184));
    ...
    if (current_step == Grid) {
        ...
        if (sypbc_button_draw(&validate_rotation)) {
            current_step++;
        }
    }
    ...
    /* Steps */
    {
        draw_push_filled_rounded_rect(&ctx->draw, 30, 105, 315, 590, 10,
    ...
    /* Loading circles */
    vec2 dims = get_text_dim(steps[(!finished ? current_step : selected_step)], ROBOTO_REGULAR16);

    /* Steps animation */
    {
        currentx_anim.target = dims.x;
        currenty_anim.target = 140 + (!finished ? current_step : selected_step) * 75 - 5;
        if (!currenty_anim.current) currenty_anim.current = 140 + current_step * 75 - 5;
        ...
        if (working) {
            ...
            draw_push_filled_rounded_rect(&ctx->draw, 90, currenty_anim.current, currentx_anim.current + 20,
    ...
    /* Steps draw */
    {
        for (size_t i = 0; i < STEPS_COUNT; i++) {
            ...
            if (finished && (!finished ? current_step : selected_step) != i &&
                is_cursor_within(ctx->input, 90, 140 + i * 75 - 5, cdims.x + 20, cdims.y + 10)) {
                ...
                if (sypbc_button_draw(&select_button)) {
                    selected_step = i;
                }
            }

            draw_push_filled_circle(&ctx->draw, 65, 150 + i * 75, 9,
                                    circle_step > i * 5 ? color_rgb(71, 123, 203) : color_rgb(184, 184, 184));
            if (circle_step == i * 5)
                draw_push_filled_circle(&ctx->draw, 65, 150 + i * 75, 9,
                                        color_rgba(71, 123, 203, steps_anim.current));

            draw_push_text(&ctx->draw, steps[i], 100, 140 + i * 75, ROBOTO_REGULAR16, color_rgb(255, 255, 255));

            if (i < STEPS_COUNT - 1)
                for (size_t j = 0; j < 4; j++) {
                    draw_push_filled_circle(&ctx->draw, 65, 170 + i * 75 + j * 12, 2,
                                            circle_step > 1 + i * 5 + j ? color_rgb(71, 123, 203) : color_rgb(184,
                                                                                                              184,
                                                                                                              184));
...
draw_load(struct sypbc *ctx)
{
    ...
    draw_push_scissors(&ctx->draw, 0, load_hide.current, WINDOW_WIDTH, WINDOW_HEIGHT);
    draw_push_filled_rect(&ctx->draw, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, color_rgb(27, 27, 27)); // Background
    draw_push_image(&ctx->draw, "logs.png", WINDOW_WIDTH / 2 - 90, 100);

    const char* welcome_txt = "Welcome to the OCR Sudoku solver from Epita, please load weights and an image to continue";
    vec2 welcome_dims = get_text_dim((char*) welcome_txt, ROBOTO_REGULAR16);
    draw_push_text(&ctx->draw, (char*) welcome_txt, WINDOW_WIDTH / 2 - welcome_dims.x / 2, 330, ROBOTO_REGULAR16, color_rgb(121, 121, 121));
    ...
    if (!load_load_button.enabled && sypbc_button_is_within(&load_load_button)) {
        ...
        draw_push_filled_rounded_rect(&ctx->draw, cpos.x, cpos.y - (dims.y + 10), dims.x + 10, dims.y + 10, 5, color_rgb(32, 32, 32));
        draw_push_text(&ctx->draw, (char*) warning, cpos.x + 5, cpos.y - (dims.y + 10) + 5, ROBOTO_REGULAR16, color_rgb(121, 121, 121));
    }
...
{
    draw_push_filled_rect(&ctx->draw, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, color_rgb(27, 27, 27)); // Background
    const char *error_text = "Your weights or image is wrong, please reselect them";
    vec2 dims = get_text_dim(error_text, ROBOTO_REGULAR16);
    draw_push_text(&ctx->draw, error_text, WINDOW_WIDTH / 2 - dims.x / 2, WINDOW_HEIGHT / 2 - dims.y / 2, ROBOTO_REGULAR16, color_rgb(121, 121, 121));
...
main(int argc, char **argv)
{
    ...
    /* Error init */
    back_button = sypbc_button_init(ctx, "Back to menu", ROBOTO_REGULAR16, WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 + 40, 200, 50, 5,
                                    color_rgb(32, 32, 32), color_rgb(255, 255, 255));

    /* Load init */
    load_load_button = sypbc_button_init(ctx, "Load image", ROBOTO_REGULAR16, WINDOW_WIDTH / 2 - 285 / 2, 400, 285, 50, 5,
                                         color_rgb(71, 123, 203), color_rgb(180, 180, 180));
    load_weights_button = sypbc_button_init(ctx, "Load weights", ROBOTO_REGULAR16, WINDOW_WIDTH / 2 - 285 / 2, 470, 285, 50, 5,
                                         color_rgb(32, 32, 32), color_rgb(255, 255, 255));
    website_button = sypbc_button_init(ctx, "", ROBOTO_REGULAR16, 10, WINDOW_HEIGHT - 60, 50, 50, 2,
                                         color_rgb(32, 32, 32), color_rgb(255, 255, 255));
    load_load_button.enabled = 0;

    /* Main init */
    load_button = sypbc_button_init(ctx, "Load image", ROBOTO_REGULAR16, 1100, 18, 145, 50, 5, color_rgb(32, 32, 32), color_rgb(255, 255, 255));
    save_button = sypbc_button_init(ctx, "Save output image", ROBOTO_REGULAR16, 45, 630, 285, 50, 5, color_rgb(32, 32, 32), color_rgb(121, 121, 121));
    select_button = sypbc_button_init(ctx, "", 0, 90, 0, 0, 0, 5, color_rgba(255, 255, 255, 15), color_rgba(0, 0, 0, 0));
    rotate_left = sypbc_button_init(ctx, "-90", ROBOTO_REGULAR16, 500 + 200, 630, 50, 50, 2, color_rgb(32, 32, 32), color_rgb(255, 255, 255));
    rotate_right = sypbc_button_init(ctx, "+90", ROBOTO_REGULAR16, 560 + 200, 630, 50, 50, 2, color_rgb(32, 32, 32), color_rgb(255, 255, 255));
    validate_rotation = sypbc_button_init(ctx, "Done", ROBOTO_REGULAR16, 620 + 200, 630, 100, 50, 2, color_rgb(71, 123, 203), color_rgb(255, 255, 255));
    save_button.enabled = 0;
    select_button.enabled = 0;
